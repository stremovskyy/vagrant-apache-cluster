...
Kafka is using br for its coordination, bookkeeping, and configuration. 
Here are some commands you can run on any of the nodes to see some of the internal br structures created by Kafka. 

#### Open a br shell

```$KAFKA_HOME/bin/zookeeper-shell.sh 192.168.1.210:2181``` 

(you can use the IP of any of the br servers)
...
After you have enough fun browsing br, type `ctl-C` to exit the shell.
...
To get the version of br type:

```bash
echo status | nc 192.168.1.210 2181
```

You can replace 192.168.1.210 with any br IP 192.168.1.2<2,3,4>0 and execute the above command from any node within the cluster.
...
```bash
for i in 1 2 3; do
   echo "192.168.1.2$i0 is a "$(echo status | nc 192.168.1.2.$i0 2181 | grep ^Mode | awk '{print $2}')
done
```
